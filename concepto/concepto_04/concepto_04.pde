...
void draw() {
  ...
  for (int y = 0; y < 16; y++) {
    ...
    // Red: (t >> 7) ^ (t + rConst)
    int rValue = ((tValue * (tValue ^ tValue % 255) | (tValue >> 4)) >> 1) + rConst;
    ...
    for (int x = 0; x < 16; x++) {
      rCore[x][y] = (rValue >> x-y) & 1; // Red bit
      gCore[x][y] = (gValue >> x-y*4) & 1; // Green bit
      bCore[x][y] = (bValue >> x-y*2) & 1; // Blue bit
    }
  }

...
float randomChance = 0.1; // Probability (0.0 to 1.0) of randomizing a bit
...
int maxPauseInterval = 100; // Max frames between pause checks
int minPauseDuration = 10;  // Min frames for a pause
int maxPauseDuration = 30;  // Max frames for a pause
...
void drawPattern() {
  ...
  for (int x = 0; x < 32; x++) {
    for (int y = 0; y < 32; y++) {
      ...
      int r = rGrid[x%16][x%16]; // (value & 4) == 4 ? 255 : 0; // Bit 2 (R)
      int g = gGrid[x%16][x%16]; // (value & 2) == 2 ? 255 : 0; // Bit 1 (G)
      int b = bGrid[x%16][x%16]; // (value & 1) == 1 ? 255 : 0; // Bit 0 (B)
      fill(r, g, b);
      rect(x * squareSize, y * squareSize, squareSize, squareSize);
    }
  }
